...
- 研究主线禁止真实账户下单。
- 当前 Phase 2 状态禁止默认进入 sandbox execution。
...
| Phase 2 | `strategies/phase2_v0_shadow_signal_research.py`、`strategies/phase2_v1_shadow_signal_research.py` | 已从 baseline signal gate 调整为交易机会模型研究；research-complete 已 close out 为 no-promotion，Phase 3 entry gate 仍失败 |
...
不要直接写 Phase 3 execution planner。不要长跑、不要 automation、不要真实订单、不要默认 sandbox execution。
