...
- 每个 Phase 至少一个、最多两个策略版本，例如 `phase1_v0_...py`，必要时再有 `phase1_v1_...py`。
...
- `phase2_v1_shadow_signal_research.py`：Phase 2 no-order live-shadow posterior collector，继承 Phase 1 selective L2 audit，强制 no-order，跟踪 candidate watches，发出 close-side `posterior_<window>` audit rows，并在 live chain Greeks 可用时写入保守的 L1-mid calendar fair-value proxy 与 Greeks/DTE/moneyness context。
...
| Phase 2 | `phase2_v0_shadow_signal_research.py` | `phase2_v1_shadow_signal_research.py` live-shadow posterior collector |
